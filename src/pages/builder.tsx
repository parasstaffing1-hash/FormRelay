      ...
      <PageHead
        ...
        actions={
          <div class="flex gap8">
            ...
            <a class="btn btn-secondary btn-sm" href={`/admin/forms/${form.id}/insights`}>Insights</a>
...
(function(){
  ...
  document.addEventListener('click', function(e){
    if(schema.version===2){
      ...
      var ea=e.target.closest('[data-ending-add]'); if(ea){ schema.endings.push({id:'ending_'+Date.now(),title:'Thank you',message:'Your response has been received.',conditions:[]}); persist(); renderMeta(); return; }
      var ed=e.target.closest('[data-ending-delete]'); if(ed){ if(!confirm('Delete this ending?')) return; var eid=ed.getAttribute('data-ending-delete'); schema.endings=schema.endings.filter(function(en){return en.id!==eid}); persist(); renderMeta(); return; }
    }
  ...
  document.addEventListener('input', function(e){
    ...
    if(!blk || !key) {
      ...
      var endingId=t.getAttribute('data-ending-title')||t.getAttribute('data-ending-message')||t.getAttribute('data-ending-redirect'); if(endingId&&schema.version===2){ var en=schema.endings.find(function(x){return x.id===endingId}); if(en){ if(t.getAttribute('data-ending-title'))en.title=t.value; if(t.getAttribute('data-ending-message'))en.message=t.value; if(t.getAttribute('data-ending-redirect'))en.redirectUrl=t.value; persist(); } return; }
  ...
  document.addEventListener('change', function(e){
    ...
    var endingDisqualified=t.getAttribute('data-ending-disqualified'); if(endingDisqualified){ var end=schema.endings.find(function(x){return x.id===endingDisqualified}); if(end){ end.disqualified=!!t.checked; persist(); } return; }
    var blk = t.getAttribute('data-blk');
    var key = t.getAttribute('data-key');
    if(!blk || !key) return;
    var b = findBlock(blk);
