...
type Props = {
  ...
  /** Public Turnstile key; the secret is never rendered into the page. */
...
export const PublicFormPage: FC<Props> = ({ form, schema, origin, errors = {}, values = {}, trust, turnstileSiteKey }) => {
  ...
  const turnstileWidget = turnstileSiteKey
    ? <div class="cf-turnstile" data-sitekey={turnstileSiteKey} data-action="form-submit" />
    : null;
            ...
            <div class="public-card">
              ...
              <div class="public-body">
                <form method="post" action={endpoint} enctype="multipart/form-data">
                  ...
                  <div class="field"><label for="name">Name</label><input class="input" id="name" name="name" type="text" placeholder="Your name" /></div>
                  <div class="field"><label for="email">Email</label><input class="input" id="email" name="email" type="email" placeholder="you@example.com" /></div>
                  <div class="field"><label for="message">Message</label><textarea class="textarea" id="message" name="message" placeholder="Your message"></textarea></div>
                  {turnstileWidget}
  ...
  return (
    <html lang="en">
      ...
      <body>
        <div class="public-wrap" style={style}>
          <div class="public-card">
            {safeCssUrl(theme.cover) ? <div class="public-cover" style={`background-image:url(${safeCssUrl(theme.cover)})`} /> : null}
            <div class="public-head">
              {safeCssUrl(theme.logo) ? <img class="public-logo" src={safeCssUrl(theme.logo)} alt="" /> : null}
              <h1>{form.name}</h1>
              {v2 && v2.settings.progressStyle !== "none" && pages.length > 1 ? (
                <><p data-progress-label>Page 1 of {pages.length}</p><div class="progress-track"><div class="progress-fill" data-progress-fill style="width:100%" /></div></>
              ) : null}
              {schema.blocks.length === 0 ? <p>This form has no fields yet.</p> : null}
            </div>
            <div class="public-body">
              <form method="post" action={endpoint} enctype="multipart/form-data" data-smart-form={v2 ? "true" : undefined}>
                <input type="text" name="_gotcha" style="display:none" tabindex={-1} autocomplete="off" />
                {trust?.startToken ? <input type="hidden" name="_started" value={trust.startToken} /> : null}
                {trust && trust.powBits > 0 ? <input type="hidden" name="_pow_challenge" value={trust.powChallenge} /> : null}
                {trust && trust.powBits > 0 ? <input type="hidden" name="_pow_nonce" value="" data-pow-bits={String(trust.powBits)} /> : null}
                {pageBlocks.map(({ page, blocks }) => (
                  <section class="page-section" data-page-id={page.id}>
                    {pageBlocks.length > 1 ? <h2>{page.title}</h2> : null}
                    {page.description ? <p class="hint" style="margin-top:-8px">{pipeText(page.description, context)}</p> : null}
                    {blocks.map((block) => <BlockField block={block} errors={errors} values={values} context={context} />)}
                  </section>
                ))}
                {turnstileWidget}
                <div class="page-actions">
                  {v2 && pages.length > 1 ? <button class="btn btn-secondary" type="button" data-prev hidden>Previous</button> : null}
                  {v2 && pages.length > 1 ? <button class="btn btn-secondary" type="button" data-next hidden>Next</button> : null}
                  <button class="btn btn-primary" type="submit" data-submit style="width:100%;height:38px">{submitText}</button>
                </div>
                {v2 ? <div class="resume-note" data-resume-note>Your progress is saved securely as you type.</div> : null}
              </form>
            </div>
            <div class="public-foot">FormRelay · Powered by {origin}</div>
          </div>
        </div>
        {v2 ? <script type="application/json" id="fr-config" dangerouslySetInnerHTML={{ __html: runtimeConfig(v2, endpoint) }} /> : null}
        {v2 ? <script src="/assets/form-runtime.js" defer /> : null}
        {trust && trust.powBits > 0 ? <script src="/assets/pow.js" defer /> : null}
      </body>
    </html>
  );
};
