...
function runtimeScript(schema: FormSchemaV2, endpoint: string): string {
  const config = JSON.stringify({ endpoint, rules: schema.logic, pages: schema.pages, variables: schema.variables }).replace(/</g, "\\u003c");
  return String.raw`(function(){
var cfg=${config};var form=document.querySelector('form[data-smart-form]');if(!form)return;
var token=localStorage.getItem('fr_resume_'+location.pathname)||new URLSearchParams(location.search).get('resume')||'';var page=0;
