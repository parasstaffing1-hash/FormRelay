...
function runtimeScript(schema: FormSchemaV2, endpoint: string): string {
  const config = JSON.stringify({ endpoint, rules: schema.logic, pages: schema.pages, variables: schema.variables, endings: schema.endings, conversational: schema.settings.conversational === true }).replace(/</g, "\\u003c");
